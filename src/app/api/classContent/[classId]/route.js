...
export async function GET(request, { params }) {
  ...
  try {
    const contentIds = await executeQueryWithRetry({
      query: `SELECT content_id FROM class_content WHERE class_id = ?`,
      values: [classId],
    });
    ...
    const contents = await executeQueryWithRetry({
      query: `SELECT * FROM content WHERE content_id IN (${placeholders}) ORDER BY created_at DESC`,
      values: contentIds.map(item => item.content_id),
    });

    const processedContents = contents.map(content =>
      content.content_type === 'text'
        ? { ...content, content_data: marked.parse(content.content_data) }
        : content
    );
