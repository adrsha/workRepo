...
export default function gradesPage() {
  ...
  const [curSelectedClass, setCurSelectedClass] = useState(null)
  ...
  function getPaymentCaller(class_id) {
    console.log(class_id)
    setCurSelectedClass(class_id)
    setShowPayer(true)
  }
  ...
  return (
    status === 'unauthenticated' || session?.user.level < 2 ? (
      <div className={styles.container}>
        ...
        {showPayer ? <Payer classId={curSelectedClass} onClose={() => setShowPayer(false)} /> : null}
      </div>
    ) : null);
}
